...
u_50_0 = ds['u'][:].sel(pressure_level=50, latitude=latitudes[0])
u_50_1 = ds['u'][:].sel(pressure_level=50, latitude=latitudes[1])
u_50_2 = ds['u'][:].sel(pressure_level=50, latitude=latitudes[2])
...
u_50_0_values = u_50_0.values
df_u_50_0 = pd.DataFrame(u_50_0_values, index=time, columns=lon)
df_u_50_0 = df_u_50_0.iloc[::-1, :]  # Invertendo o tempo
...
# Criando o gráfico
d = ax[0, 0].contourf(df_u_50_0.columns,
                       df_u_50_0.index,
                       df_u_50_0.values,
                       cmap=cmap,
                       norm=norm0,
...
ax[0, 0].set_title(f'A) Latitude: {latitudes[0]} u zonal (m/s) - 50 hPa', fontsize=12)
...
time_range = pd.to_datetime(df_u_50_0.index)
...
u_50_1_values = u_50_1.values
df_u_50_1 = pd.DataFrame(u_50_1_values, index=time, columns=lon)
df_u_30_1 = df_u_50_1.iloc[::-1, :]  # Invertendo o tempo
# Definindo normalização para o colormap



c = ax[0, 1].contourf(df_u_50_1.columns, df_u_50_1.index,
                       df_u_50_1.values, cmap=cmap, norm=norm1,
                         extend='both', levels=np.arange(-30, 3, 3))
ax[0, 1].set_title(f'B) Latitude: {latitudes[1]} u zonal (m/s) - 50 hPa', fontsize=12)
...
u_50_2_values = u_50_2.values
df_u_50_2 = pd.DataFrame(u_50_2_values, index=time, columns=lon)
df_u_50_2 = df_u_50_2.iloc[::-1, :]  # Invertendo o tempo



# Plotando o terceiro subplot
e = ax[1, 0].contourf(df_u_50_2.columns, df_u_50_2.index, df_u_50_2.values,
                       cmap=cmap, norm=norm2, extend='both', levels=np.arange(-30, 3, 3))
ax[1, 0].set_title(f'C) Latitude: {latitudes[2]} u zonal (m/s) - 50 hPa', fontsize=12)
...
# Salvando o gráfico
plt.savefig(DIRFIG + 'hov_u_2x2_50.png', dpi=300, bbox_inches='tight')
